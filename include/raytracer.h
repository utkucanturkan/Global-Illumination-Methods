...
class RayTracer {
  public:
                ...
                for (auto& e : _scene->intersect(ray)) {
                    ...
                    if (e->intersect(ray, intersection, normal)) {
                        if (intersection[0] < tnear) {
                            ...
                            float lightIntensity = 1.0, diffuse_light_intensity = 0,
                                  specular_light_intensity = 0;

                            glm::dvec3 light_dir = glm::normalize((_light - intersection));
                            float light_distance = glm::length(_light - intersection);

                            // SHADOW
                            glm::dvec3 shadowOrigin = (glm::dot(light_dir, normal) < 0)
                                                          ? intersection - normal * 1e-3
                                                          : intersection + normal * 1e-3;

                            float shadowTNear = INFINITY;
                            bool isShadow = false;
                            float shadowT0;
                            glm::dvec3 shadowIntersection, shadowNormal;
                            for (auto& e : _scene->intersect(ray)) {
                                shadowT0 = INFINITY;
                                shadowIntersection = {shadowT0, 0, 0};
                                Ray shadowRay(shadowOrigin, light_dir);
                                if (e->intersect(shadowRay, shadowIntersection, shadowNormal)) {
                                    if (shadowIntersection[0] < tnear) {
                                        shadowTNear = shadowIntersection[0];
                                        glm::dvec3 tempShadowDir = shadowRay.dir;
                                        shadowIntersection = shadowRay.origin +
                                                             tempShadowDir.operator*=(shadowTNear);
                                    }
                                }
                            }

                            if (glm::length((shadowIntersection - shadowOrigin)) < light_distance) {
                                isShadow = true;
                            } else {
                                isShadow = false;
                            }
                        ...
                        if (fabs(ray.dir[1]) > 1e-3) {
                            float d = -(ray.origin[1] + 1) /
                                      ray.dir[1]; // the checkerboard plane has equation x = -5
                            glm::dvec3 tempRayDir = ray.dir;
                            ...
                            if (d > 0 && fabs(pt[0]) < 10 && pt[2] < 5 && pt[2] > -30 &&
                                ...
                                color =
                                    (int(.5 * intersection[0] + 1000) + int(.5 * intersection[2])) &
                                            1
                                        ? glm::dvec3(1, .6, .1)
                                        : glm::dvec3(1, 1, 1);
                            }
    ...
    glm::dvec3
    refract(const glm::dvec3& I, const glm::dvec3& N, const float eta_t, const float eta_i = 1.f) {
        // Snell's law
